...
@register_adv_est(AdvantageEstimator.GRPO_OFF_POLICY)
def compute_grpo_off_policy_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    ...
    with torch.no_grad():
        ...
        for idx in id2score:
            ...
            if idx in off_policy_stats:
                ...
                for i, score_idx in enumerate([j for j, x in enumerate(index) if x == idx]):
                    if norm_adv_by_std_in_grpo:
                        id2adv[score_idx] = (scores[score_idx] - mu_alpha) / (sigma_alpha + epsilon)
                    else:
                ...
                if len(group_scores) > 1:
                    ...
                    sigma_k = torch.std(group_scores)
                    for i, score_idx in enumerate([j for j, x in enumerate(index) if x == idx]):
                        if norm_adv_by_std_in_grpo:
                            id2adv[score_idx] = (scores[score_idx] - mu_k) / (sigma_k + epsilon)
                        else:
...
def compute_grpo_adaptive_filter_advantage(
    ...
    **kwargs,
    ...
    return full_advantages, full_advantages
...
def compute_policy_loss(
    ...
    pg_losses2 = -advantages * torch.clamp(ratio, clip_center - cliprange_low, clip_center + cliprange_high)  # - clip(ratio, 1-cliprange, 1+cliprange) * A
...
def compute_importance_weights(current_log_probs: torch.Tensor, 
    ...
    # Log importance weights (response part only)
    log_importance_weights = (current_log_probs - rollout_log_probs) * response_mask

    # Sum over sequence to get total log weight per sample
    ...
    # Clip weights for training stability
    if clip_ratio > 0:
...
def compute_grpo_off_policy_importance_sampling_advantage(
    ...
    **kwargs,
    ...
    """
    GRPO advantage with importance sampling.

    Args:
        token_level_rewards: token-level rewards [batch_size, seq_len]
        response_mask: response mask [batch_size, seq_len]
        index: sample index/uid for grouping
        current_log_probs: current policy log probs [batch_size, seq_len]
        rollout_log_probs: rollout policy log probs [batch_size, seq_len]
        epsilon: numerical stability
        norm_adv_by_std_in_grpo: whether to normalize advantage by std
    ...
    with torch.no_grad():
        ...
        for idx in id2score:
            ...
            if len(group_scores) == 1:
                ...
                # Weighted mean and std with importance weights
        ...
        # Apply importance-weighted advantage
        for i in range(bsz):
            if norm_adv_by_std_in_grpo:
