...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def clear_temp_override(self):
        """Clear temp override and restore state."""
        self._temp_override = None
        self.restore_state()
    
    @property
    def effective_should_update_params(self):
        """Effective param update flag (considering temp override)."""
    ...
    def set_param_update_control(self, should_update: bool, current_step: int = None):
        """Control whether to update vLLM params on next __enter__."""
        if self._temp_override is None:  # Only update when no temp override
