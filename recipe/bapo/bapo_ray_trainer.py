...
Optimized version with immediate old_log_prob computation after generation.
"""
...
class RayBAPOTrainer(RayPPOTrainer):
    """
    RL trainer with off-policy support.
    """
    ...
    def fit(self):
        """
        The training loop combining off-policy strategies.
        Fixed version with proper variable initialization and logic flow.
        """
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    # Immediately compute old_log_prob after generation (before any filtering)
                    ...
                    # Step 4: Apply off-policy filter
                    with _timer("offpolicy_filter", timing_raw):
                        enable_filtering = self.config.algorithm.filter_groups.get("enable", True)
                        
                        if enable_filtering:
                            max_ratio = self.config.algorithm.get("offpolicy_filter_ratio", 1.0)
                            new_batch_filtered = self.off_policy_manager.filter_current_batch_samples(
                                new_batch, 
                                max_ratio=max_ratio
                            )
                            
                            # Calculate keep ratio for metrics
                            original_size = len(new_batch)
                            filtered_size = len(new_batch_filtered)
                            offpolicy_keep_ratio = filtered_size / original_size if original_size > 0 else 0.0
                            
                            print(f"OffPolicyManager filter: kept {filtered_size}/{original_size} samples ({offpolicy_keep_ratio:.2%})")
                        else:
                            new_batch_filtered = new_batch
                            offpolicy_keep_ratio = 1.0
                            print("OffPolicyManager filtering disabled, using original batch")

                    # Step 4.1: Accumulation logic (similar to DAPO but using off-policy filter)
                    enable_accumulation = self.config.algorithm.filter_groups.get("enable", True)
                    ...
                    if enable_accumulation:
                        ...
                        if num_prompt_in_batch < prompt_bsz:
                            ...
                            max_num_gen_batches = self.config.algorithm.filter_groups.get("max_num_gen_batches", 10)
                            if max_num_gen_batches <= 0 or num_gen_batches < max_num_gen_batches:
                                print(f"{num_gen_batches=}. Keep generating...")
                                continue
                            else:
                                offpolicy_keep_ratio = num_prompt_in_batch / prompt_bsz
                        else:
                            # Align the batch
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        if self.off_policy_manager.enable_off_policy_samples:
                            ...
                            # 6.1. Using filtered data as the initial batch
                            current_batch_filtered = batch
                            print(f"OffPolicyManager kept {offpolicy_keep_ratio:.2%} data in the initial batch")
                            
                            if len(current_batch_filtered) > 0:

                                # current_batch_filtered.batch["clip_center"] = torch.exp(current_batch_filtered.batch["old_log_probs"] - current_batch_filtered.batch["old_log_probs"])
                                current_batch_filtered.batch["clip_center"] = torch.ones_like(current_batch_filtered.batch["old_log_probs"])
                            ...
                            # 6.2. Add promoted batch if available
                            if remaining_needed > 0 and promoted_batch is not None:
                                ...
                                if torch.sum(non_duplicate_mask) > 0:
                                    ...
                                    # promoted_batch_filtered.batch["clip_center"] = torch.exp(promoted_batch_filtered.batch["old_log_probs"] - promoted_batch_filtered.batch["old_log_probs"])
                                    promoted_batch_filtered.batch["clip_center"] = torch.ones_like(promoted_batch_filtered.batch["old_log_probs"])
                            ...
                            promoted_count = len(promoted_batch_filtered) if promoted_batch_filtered is not None else 0
                            still_needed = remaining_needed - promoted_count
                            
                            # 6.3. Add target buffer samples if needed
                            if still_needed > 0 and self.off_policy_manager.enable_completion:
                                ...
                                if available_groups > 0:
                                    ...
                                    groups_needed = still_needed // rollout_n
                                    
                                    buffer_batch = self.off_policy_manager.sample_filtered_groups(
                                        min(groups_needed, available_groups)
                                    )
                                    print(f"Available groups: {available_groups}, groups needed: {groups_needed}, groups sampled: {len(buffer_batch) if buffer_batch else 0}")
                                    if buffer_batch is not None:
                                        ...
                                        if torch.sum(non_duplicate_mask) > 0:
                                            ...
                                            # Add required metrics for DAPO compatibility
                                            metric_name = self.config.algorithm.filter_groups.get("metric", "seq_final_reward")
                                            ...
                                            cur_pi_log_prob = self.actor_rollout_wg.compute_log_prob(buffer_batch_filtered)
                                            cur_pi_log_prob.batch.pop("entropys", None)
                                            buffer_batch_filtered.batch["clip_center"] = torch.exp(cur_pi_log_prob.batch["old_log_probs"] - buffer_batch_filtered.batch["old_log_probs"])
                            ...
                            if len(batches_to_combine) == 0:
                                ...
                                final_batch = DataProto.concat(cleaned_batches)
                    ...
                    if len(collected_batches_for_buffer) > 0:
                        ...
                        self.off_policy_manager.collect_data(combined_buffer_batch, combined_is_off_policy, step=self.global_steps)
                ...
                # metrics specific to off-policy training
                metrics.update({
                    "train/num_gen_batches": num_gen_batches,
                    "train/keep_ratio": offpolicy_keep_ratio,
                })
